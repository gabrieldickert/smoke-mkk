...
- **UX audit changes (F10, 2026-09-25), these override the older bullets below where they differ:**
  ...
  - *Organic divider line (owner request, 2026-09-26: "maybe the line should be more organic instead of straight"):* all three dividers' 2 px straight line becomes a softly wavering thread, like a drawn-out wisp of smoke:
    - **Shape:** a smooth wavy curve, low amplitude (±3–5 px), long gentle undulations (several hundred px), built from 2–3 overlaid sine harmonics with random phases/wavelengths per divider and load (seeded like the rest of the smoke), so no two dividers and no two loads look the same. Thickness varies organically along the line (≈ 1–3 px, thinning in places almost to nothing, never a hard break), drawn as a filled shape between two offset curves, not a constant-width stroke. Same violet → rose → violet gradient, same fade towards both ends, same soft glow and slow pulse as before.
    - **Motion:** the wave drifts slowly sideways (tens of seconds per cycle), seamlessly: the curve is periodic, the SVG is one period wider than the divider, and only its `transform: translateX` animates by exactly one period. No per-frame path morphing, no SVG filter animation; the glow is a static blurred copy whose opacity pulses (opacity only, as before).
    - **Smoke rim:** stays as it is (straight strips centred on the line's mean); the amplitude is small enough that the smoke still hugs the thread. The rim must still read as coming from the line.
    - **Build:** one small component `components/SmokeDivider.vue` (inline SVG, path generated once on mount from the seeded harmonics, its own `smokeRimStyle()`), replacing the three hand-copied divider blocks in `HomeView.vue` (×2) and `App.vue` (×1); the now-unused straight-line CSS (`.glow-divider` background, the blurred `::after`) is deleted from `main.css`, `.divider-smoke` stays. Keep each divider's current placement and z-order. `aria-hidden`, `pointer-events: none`, no layout shift, no horizontal overflow.
    - **Reduced motion:** the wave is static (no drift), glow static, rim static.
    - **F21 outcomes accepted by the PM (2026-09-26):** period P 800–1200 px (random), all harmonics whole cycles per P, path sampled every 8 px and 1.5 × the largest screen side + P wide (`// ponytail:` ceiling), drift 12–20 px/s in a random direction; the fixed-in-place gradient is two copies of the thread (violet with the end fades, rose masked 20/50/80 % on top) drifting in sync; the glow sits *under* the thread (on top it washed out the hairline stretches); the rim strips now overlap across the line (`--rim-in: 6px`, fading in over 12 px) because without the old straight line their meeting edge showed as a hard seam. Rainbow bands above the pitch ↔ `#standorte` divider come from the aurora on all sections, not from the dividers.
  - *Footer divider (owner request, 2026-09-26):* the footer's plain `border-t` (between the map section and the footer on `/`, between the legal text and the footer on `/impressum`, `/datenschutz`) is replaced by the same glowing divider with its smoke rim (§4.3 "Smoking dividers", third version): same classes, same markup, its own `smokeRimStyle()` so it never moves in sync with the other two. Lives in `App.vue`, so it shows on every page. The footer's top padding may grow so the lower smoke strip does not crowd the footer text (links stay fully legible and clickable; the rim is `pointer-events: none`). No new CSS unless unavoidable.
  - *Aurora on all sections (trial 2026-09-25, **kept by the owner 2026-09-26**, no longer a trial):* the hero's aurora backdrop runs behind the whole home page: one aurora layer, `position: fixed` behind the content (not one copy per section — each is a blurred, blended, animated full-size layer), sections without their own opaque background so it shows through; map and panel keep their card/map surfaces so text contrast is unchanged. Lives in one class (`aurora-everywhere` on `HomeView`'s root) and its block in `main.css`.
...
- **Superseded by "Lighter map" below (owner, 2026-09-26)** for the tile filter and tint values; the container background, control theming and marker rules above still hold.
...
**Lighter map (owner request, 2026-09-26).** The dark map is hard to read: `invert(1)` turns OSM's white streets on light land into near-black lines on dark grey (roads darker than land), and `brightness(0.5)` crushes the rest. No monotonic filter can fix that while the invert stays. So: drop the invert, keep the OSM tiles (no new host, no Datenschutz change) and recolour in CSS only:
- Tile pane: grayscale plus a mild brightness/contrast pull, so land reads as a muted light-to-mid lavender grey, streets are the lightest lines, and labels stay dark and legible at zoom 10–16. No invert.
- Brand tint: the existing `::before` over the tile pane only, with a violet from the tokens and `mix-blend-mode: color` (or `multiply` at low strength), so the map clearly belongs to the violet palette without going muddy.
- Fits the dark page: the rounded map card should look like an intentional lighter surface, not a white hole. It must not glare next to `--background`.
- Markers go **darker** as the map goes lighter (owner, 2026-09-26): the light-lavender `--secondary` dots measured 1.2:1 and the `--accent` rose 1.1:1 against the F18 land, so a rim alone is not enough. The fills move to deeper brand shades (violet ≈ `--primary` or darker, a deeper rose for the selected dot), each ≥ 3:1 against the land (non-text UI), and they still read as the neon brand pins (glow kept, tuned so it doesn't wash out on the lighter base). New colours are tokens in the `.dark` block, not literals. The "Dein Standort" dot and the nearest ring follow the same rule. Tooltips stay dark (≥ 4.5:1 text).
- The zoom controls and attribution stay token-themed; the attribution stays ≥ 4.5:1.
- Final values (F18): tiles `grayscale(1) brightness(0.7) contrast(1.5)`; tint `--map-tint` at `opacity: 0.6`, `mix-blend-mode: color`; land ≈ #9389b0, labels 6.4:1. The map class is `.map-brand` (was `.map-dark`). Marker tokens: `--marker` #4e09ae (idle pins, nearest ring, "Dein Standort" ring), `--marker-selected` #7f0a28, glows `--primary` / `--accent` at 80 %; every fill 3.26:1 on the land, the white 2 px stroke stays; selected ring `ring-accent/50`. Marker keyboard focus uses `--background` (5.8:1), because `--ring` is only 1.2:1 on the lighter land (PM accepted). Ceiling: at ≥ 3:1 the selected "rose" reads as crimson and the pink comes from the halo. A pinker pin needs a lighter land or a lower threshold (owner's call).
...
**Section smoke reveal (owner request, 2026-09-26: "when scrolling down to the section where the vendor machines are … a cloud dissolve animation just like when entering the website").** The map section `#standorte` gets its own smaller replay of the intro when it is first scrolled to:
- **When:** once per full page load, the first time `#standorte` comes into view (one native `IntersectionObserver`, fires when roughly a third of the section is visible; disconnected after it fires). Scrolling back up and down never replays it; SPA navigation away and back to `/` does not replay it either (module-level flag, no storage). Skipped (flag set, nothing drawn) when the page-load intro is still up at that moment — direct load of `/#standorte`, or the hero CTA clicked during the intro — so smoke never plays twice over the same view.
- **Where:** covers the section only (heading, map, panel), not the viewport: an absolutely positioned layer inside `#standorte`, above the Leaflet panes, below the sticky header (z 1200) and the dialog. The header never gets smoked.
- **Look:** the intro's look scaled to the section box: puffs blown outward from the section's centre, growing and thinning, ≈ 1.2–1.4 s, section legible by ~1 s; fewer puffs than the intro (≈ 20–24) sized relative to the section. **No visible rectangle:** the section sits on the aurora and the page background, so a flat opaque base would show hard edges — the base (if kept) and the smoke fade softly towards all four section edges (mask), or the base is dropped if the puffs alone read as a cloud. Same randomness rules as the intro.
- **Build:** reuse `SmokeIntro.vue` — no second smoke renderer, sprites from the shared `src/smoke.ts` cache. Minimal generalisation, e.g. one prop that switches the layer from `fixed` viewport to `absolute inset-0` in its parent (canvas sized to its own box, no hold). Mounted with `v-if` inside `#standorte` in `HomeView.vue`, unmounted on `done`. The age-gate backdrop rule in `main.css` (`html:has(.smoke-intro)`) must not react to the section variant.
- **Never blocking:** `pointer-events: none`, `aria-hidden`; map, list and focus (CTA / caret land here) work during the dissolve. Idle = no work: no rAF before it fires or after it ends.
- **Reduced motion:** not rendered.
- **F22 outcomes accepted by the PM (2026-09-26):** `SmokeIntro.vue` `section` prop (22 puffs, 1200 ms, aspect-following grid, ~100 ms fade-in, class `smoke-section` so the age-gate rule ignores it); trigger `rootMargin: 0 0 -33% 0` (a "third of the section" threshold never fires for the 2583 px phone section); layer capped at `125svh` so on phones the cloud stays sized to the visible part. **Superseded by the owner (2026-09-26: "the animation should occur before the map is displayed"):** the map must never be seen uncovered before the reveal. The cloud is already in place, thick and holding (the intro's `hold`), when the section scrolls into view, and dissolves from there: a second observer with a positive bottom `rootMargin` (≈ half a viewport ahead) mounts the section smoke with `hold` as the section approaches, and the existing `-33%` trigger releases `hold`, which starts the dissolve. The ~100 ms fade-in goes away, because the cloud is opaque from its first paint. If the visitor turns back before the dissolve, the held cloud unmounts when it leaves the approach zone (no rAF off screen) and remounts on the next approach. The flag is set only when the dissolve starts. An instant hash or CTA jump that fires both observers at once mounts and dissolves in the same step: the base is opaque on the first paint, so the map still isn't seen first. Accepted with the rework: `SmokeIntro.vue` pauses its loop while a held section cloud is entirely off screen (an IntersectionObserver on the layer), because `#standorte` starts at the fold and is therefore inside the approach zone from page load: rAF 0/s until the cloud is seen. The unmount on leaving the approach zone stays in the code but is unreachable on current layouts; the pause covers the no-idle-work rule. Resize via a ResizeObserver on the canvas (a held cloud outlives the section growing when the list arrives). Mask fades match the section padding (1rem sides, 4rem top/bottom; `xl` reaches 3rem into the margin), and the 125svh cap applies below `md` only. Found in review: 2 of 6 shared sprites keep up to 16 % alpha on their square border, which shows as a straight line in every smoke (intro and scroll wisps too). F22 is extended to fix it in `smoke.ts` `renderSprite` (alpha forced to 0 at the inscribed circle) and to add `.smoke-section` to the reduced-motion hide rule in `main.css`.

**UX audit 2 (PM, 2026-09-26; tickets F23–F25).** Second critical pass over the running stack at 1280×800 and 375×812. Anything animated was measured in headless Chrome over CDP (the browser pane throttles `requestAnimationFrame` to ~2 fps); the probe is `scratchpad/audit2.mjs` next to the F10 scripts (dev machine only, not in git) and F25 reuses it for its before/after numbers. Findings, each with the decided fix; nothing here changes §5.

1. **Pin pile-up (F23).** With the 24 Vendon sites the overview is unreadable: at the fit-all zoom 18 of 24 pins (desktop, zoom 9) and 19 of 24 (phone, zoom 8) have a neighbour within 32 px; Bad Orb's five sites are 0–3 px apart there, still 5–20 px apart at the select zoom 12 (Jahnstraße ↔ Würzburger Straße 6 px, so after selecting one the next cannot be tapped), and only separate at zoom 15. Fix in `MachineMap.vue`, raw Leaflet, no plugin (the §10 trigger for clustering was a count; density hit first):
   - *Select zoom:* selecting a location zooms to the smallest zoom in 12…16 at which the selected pin's nearest other pin is ≥ 40 px away (`map.project(latlng, z)` for every pin), never below the current zoom; deselecting still fits all. `SELECT_ZOOM` becomes that function.
   - *Grouping at the overview (own minimal clustering):* after every `zoomend` and on each render, pins whose projected distance at the current zoom is < 28 px are grouped (greedy: walk the pins west → east, attach each to the first group whose centroid is within 28 px, else start a group). No grouping at zoom ≥ 16 (nothing in §3 still overlaps there). A group of 2+ renders as **one** divIcon at its centroid: the idle dot enlarged to 28 px in `--marker` with `map.clusterCount` (`{n}x`, owner 2026-09-26: like the hero's `39x`, not a bare number) centred inside (`text-xs font-bold`, `--primary-foreground`, ≥ 3:1 on the fill; the pin grows into a pill with `min-width: 28px` and side padding if the label needs it), glow and 2 px stroke kept; tooltip = `map.cluster` (`{n} Standorte`) as a bold first line, then **one member display name per line** (muted, smaller; owner 2026-09-26: the one-line list for Bad Orb's five sites was far too long), tooltip `max-width` ≈ 16rem with normal wrapping; `aria-label` = `{n} Standorte: ` + the names joined with `, `; keyboard-focusable like a pin (`role="button"`); click / Enter → `fitBounds` of the members with `padding [40, 40]`, `maxZoom 16`, which separates them or splits the group at the next `zoomend`. If the selected location is inside a group, the group takes the selected colour (`--marker-selected`, rose ring) so the selection stays visible; if the nearest location is inside a group, the group carries the dashed nearest ring. Single pins keep everything as today plus the address line (§6 `map.pinAddress`: tooltip line 2 = `street` with house number, muted, smaller; also appended to the `aria-label`); the visitor dot is never grouped. The drop-in plays once over whatever is rendered at the first reveal (groups included); regrouping after a zoom never replays it. `// ponytail:` greedy O(n²) per `zoomend`, fine below ~200 pins; a grid index if that is ever exceeded.
2. **Phone: the map block stays invisible (F24).** The `Reveal` around map + panel is 2 279 px tall on a phone and fires at `amount: 0.2` = 456 px visible; at the section's landing position (top under the header) only 17 % is visible, so the visitor sees the heading over an empty dark area (`blockOpacity 0`, screenshot `a2-map-375.png`) until they scroll ~70 px further and the map pops in. Fix: `Reveal` gets an optional `amount` prop (default 0.2, motion-v's `'some' | 'all' | number`); the map + panel block passes `amount="some"`.
3. **Phone: selection scroll (F24).** After a marker or list tap the panel is scrolled into view, so the map with the pin just selected leaves the screen (map bottom at 31 px, measured), and the panel lands 9 px under the 65 px header (top 56 px). New rule below `md`: scroll the **map card** to the top (`scroll-mt-20` on the map wrapper, `block: 'start'`); the map (318 px) plus the detail header (name, address, Route, picker ≈ 200 px) fit in the 748 px below the header, so pin and stock header are seen together (`a2-badorb-375.png` shows that framing). `scrollToMap` for the CTA and the caret is unchanged; the focus rules of F10 stay.
4. **Keyboard: 28 Tab stops between the map and the list (F24).** From the map container to the search field: 24 pins, zoom ±, two attribution links. Fix (WCAG 2.4.1 bypass block, like the page's skip link): a link `map.skip` placed in DOM order directly before the map wrapper, visually hidden until focused (`sr-only focus:not-sr-only`, same look as the skip link, absolutely positioned at the map card's **top-right** — the zoom control owns the top-left — above the Leaflet panes); `href="#machine-search"`; its click handler focuses `#machine-search` in the list view, otherwise the `Alle Automaten` button. Pins stay focusable.
5. **Location without feedback (F24).** With the browser position denied, unavailable or timed out, the hero CTA only scrolls: status line and panel text are byte-identical before and after (measured) — a silent failure, and there is no busy state during the up-to-10 s wait. Fix: `HomeView` keeps `geoState: 'idle' | 'locating' | 'ok' | 'failed'` (set on every `locate()` call and its callbacks) and passes it to the panel; the panel's **list view** shows one muted line directly under the search field: `geo.locating` while pending, `geo.unavailable` after a failure (stays until the next attempt or a position arrives), nothing when idle or ok. The visible `search.count` line (owner request, 2026-09-26, see §6) shares that spot: one small muted block under the field, count line first, geo line below it, each only when it applies, no layout jump when one appears (the block reserves nothing but never overlaps the list; the list simply starts below it). The persistent `role="status"` line announces both, joined with ` · ` (count first). The detail view shows nothing extra (on the CTA path a success selects the nearest location; a failure leaves the visitor on the list, where the line is).
6. **Hero headline blank a third of the time (F24).** FlipWords' leave (0.6 s) and enter (≈ 0.6 s) run every `duration` = 2.5 s, so the h1 is empty or half-drawn ≈ 40 % of the time; the screenshots at both widths caught an empty h1. Fix: `:duration="4000"` (readable ≈ 70 %); no component edit.
7. **F14 folded in (F24):** the legal `<h1>` (`Datenschutzerklärung`, 30 px) fills the whole 343 px column at 375 and overflows at 320 → `text-2xl` below `sm`, `hyphens-auto break-words` (`lang="de"` is set).
8. **The idle page never rests (F25).** At rest it repaints continuously: the aurora (`background-position` keyframes on a blurred, difference-blended full-viewport layer, fixed behind the whole page), three drifting threads, six rim strips with animated `mask-position`, the glow pulse, FlipWords. Headless Chrome with 4× CPU throttling (≈ a mid-range phone): 44 fps idle at 1280×800 (top) / 54 (map), 69 fps at 375×812 (top) / 66 (map) — about 14 ms of work per frame while nothing happens, on top of scrolling and the smoke; page load has 80–120 ms long tasks (sprites and tiles). Fix without visual change: each `SmokeDivider` pauses all its animations while it is off screen (`IntersectionObserver`, `rootMargin: '100px'` → class `paused` → `animation-play-state: paused` on thread drift, glow pulse, rim flow, reach and sway), so at most two dividers animate at a time (none at the map on a phone). `FlipWords` and the aurora are untouched (the aurora on all sections is kept, owner 2026-09-26). Gate: the same probe at 4×, 375×812, idle at the map section ≥ 66 fps before → higher after; report before/after at both widths, top and map.

**F23–F25 outcomes accepted by the PM (2026-09-26):**
- *Grouping:* closest pair merged first (agglomerative), not the first-fit west → east walk above: the walk chained Wächtersbach into Bad Orb (a 6) and did not guarantee the 28 px spacing. Same O(n²) ceiling. Measured: 1280×800 zoom 9 → 14 pins, Bad Orb `5x`, min 28 px; 375×812 zoom 8 → 10 pins, min 29 px.
- *Phone overview:* at zoom 8 Bad Orb, Wächtersbach, Hesseldorf, Lettgenbrunn, Mernes and Jossgrund are one `10x` (they are 14 px apart there; no grouping keeps Bad Orb a separate `5x` at 28 px). One tap → zoom 10 → `5x` / `3x` / `2x`. Accepted; a closer phone overview zoom would cut off Alsfeld/Rodgau.
- *Keyboard:* Enter/Space activate pins and groups (Leaflet 1.9.4 sets `role="button"` but never activates from the keyboard); after a keyboard-opened group splits, focus moves to its first member. Tooltip text is HTML-escaped.
- *Skip link:* sits before the map in DOM order, so it is the next Tab after `#standorte` (where CTA and caret put focus) and Shift+Tab from the map container.
- *Status line* still announces `0 Standorte` on no match (visible: only `search.noResults`).
- *F25:* `.paused` needs `!important` (the divider rules set the `animation` shorthand). Idle fps at 4× CPU, same build A/B: 1280 top 51 → 71, map 51 → 98; 375 top 36 → 48, map 42 → 84.
- `geo.locating` flashes for a few ms when the browser answers instantly (no permission prompt); no show delay added.

**Location retry (owner request, 2026-09-26; F27).** A visitor who declined the location by accident must be able to turn it back on. Browser rule: after "Block" (`PERMISSION_DENIED`, error code 1) the page cannot prompt again — every `getCurrentPosition` fails instantly until the visitor re-allows the site in the browser's site settings; a merely dismissed prompt, a timeout or "position unavailable" can be retried with a new prompt. So:
- `geoState` gains `'denied'` (error code 1, or `permissions.query` reports `denied`), next to `'failed'` (codes 2/3).
- Panel list view, the geo line under the search field: `'failed'` → `geo.unavailable` + a small text button `geo.retry`; `'denied'` → `geo.blocked` + the same `geo.retry` button. The button calls the CTA's path (`locate(true)`: on success it selects the nearest location as before). Button: text style in `--secondary`, underline on hover, ≥ 44 px tall hit area (padding), visible focus; the status line announces the new geo text as before.
- `HomeView` subscribes once to the `PermissionStatus` `change` event (from the `permissions.query` it already makes on mount): when it turns `granted` after a failure or denial, it locates again — `locate(true)` if the last attempt came from the CTA or the retry button, else `locate(false)` (mark and sort only). No polling, listener removed on unmount. Browsers without the Permissions API (older Safari) rely on the retry button.
- No storage, no new component; the Datenschutz TODO comment is unchanged (same data, same use).

*Accepted, no ticket:* the section smoke (F22) adds ≈ 1.4 s before the map is usable on the CTA path (owner's request); the legal placeholders are a launch blocker for the operator, not a UI defect; the list rows on phones are 60–84 px tall because long site names and addresses wrap (readable, left as is).
...
| section.locations | `Unsere Standorte` (owner, 2026-09-26, F26; was `Unsere Automaten`; the anchor stays `#standorte`; also the `aria-label` of the first-screen caret) |
...
| panel.title | `Alle {n} Standorte` (owner, 2026-09-26, F26: n = all active locations, currently 24; a fixed total that does **not** change while searching — the matches are `search.count`; `Alle Standorte` until the list has loaded; was `Alle Automaten` (owner, 2026-09-25), was `Aktueller Bestand`, which misled above the machine list). The `<h3>` of the panel's list view only; in the detail view the `panel.back` button carries the same words and the machine's display name is the `<h3>` |
...
| search.count | `{n} Standorte` (singular `1 Standort`; counts list rows = locations, F15). Since F24 (owner, 2026-09-26) also **visible**: a muted line directly under the search field while the query is non-empty and has ≥ 1 match; 0 matches keeps `search.noResults` instead |
...
| map.pinAddress | second, smaller muted line in every single-pin tooltip: the location's `street` as stored, i.e. with its house number (e.g. `Niesiger Str. 69`); omitted when `street` is `""` (owner, 2026-09-26; F23). The pin's `aria-label` appends it after ` · ` |
| map.cluster | `{n} Standorte` (bold first line of a grouped pin's tooltip, then one member display name per line; `aria-label` `{n} Standorte: name, name, …`; F23) |
| map.clusterCount | `{n}x` (label inside a grouped pin, e.g. `5x`; F23) |
| map.skip | `Karte überspringen` (bypass link before the map, visible on focus; F24) |
| geo.locating | `Standort wird ermittelt …` (muted line under the search field while the browser position is pending; F24) |
| geo.unavailable | `Kein Standort freigegeben. Such per PLZ oder Ort.` (same line after error or timeout; F24; since F27 not after a permanent denial) |
| geo.blocked | `Standort ist im Browser blockiert. Erlaub ihn über das Symbol links neben der Webadresse.` (same line after a permanent denial; F27) |
| geo.retry | `Nochmal versuchen` (text button after `geo.unavailable` / `geo.blocked`; F27) |
...
8. Map: 24 locations as brand-coloured pins, overlapping sites grouped into count pins at the overview (Bad Orb = one `5x`; F23), hero tagline reads `39x`, no broken images, all inside the initial viewport; click a group → zooms until it splits; click a pin → map zooms until no other pin is within 40 px, panel shows name, address, Route, "Stand" and grouped items with the three stock states; pick a machine in the panel list → same; Schlüchtern shows the `Welcher Automat?` picker with five options, switching loads that machine's stock; Fulda shows no picker; `Alle Automaten` → back to the list and the overview.
...
10. 375 px width: hero, map, panel stack; no horizontal scroll; the map block is visible as soon as the section is reached; a one-finger swipe over the map scrolls the page until the map is tapped; a selection scrolls the map card under the header with the pin and the stock header on screen (F24). Keyboard: dialog buttons, markers, panel list and footer links reachable; Tab from the map offers `Karte überspringen`, Enter lands in the search field; focus visible.
...
| **F18** | smokemkk-frontend | `frontend/src/assets/main.css`, `frontend/src/components/MachineMap.vue` (class name, marker colours/outline/glow) — `ui-ux-pro-max` first; §4.3 "Lighter map". No new string, no new dependency, no new tile host. | `npm run build` clean; browser at 1280×800 and 375×812 against the running API: streets clearly visible as the lightest lines, place and street labels readable at the default overview zoom and after selecting a location (zoomed in), map reads as violet-tinted and sits well on the dark page, marker fills (idle, selected, your location, nearest ring) each ≥ 3:1 against the land and visibly darker than before, tooltips readable, attribution ≥ 4.5:1; report the final filter and tint values with a screenshot at both zooms; console clean | — |
...
| **F20** | smokemkk-frontend | `frontend/src/App.vue` only — `ui-ux-pro-max` first; §4.3 "Footer divider". `main.css` and `MachineMap.vue` are being edited by F18 in another session: do not touch them; if CSS is unavoidable, stop and report. No new string, no new dependency. | `npm run build` clean; 1280×800 and 375×812 on `/`, `/impressum`, `/datenschutz`: the footer's top edge is a glowing divider with the outward-flowing smoke rim, looks like the other two but moves independently, footer text and links fully legible and clickable, no horizontal scroll (the known F14 overflow on `/datenschutz` at 320 aside), reduced motion → static rim; console clean | F12 |
| **F21** | smokemkk-frontend | new `frontend/src/components/SmokeDivider.vue`, `HomeView.vue`, `App.vue`, `main.css` (delete the straight-line divider CSS only), `smoke.ts` if the seeded random helper lives there — `ui-ux-pro-max` first; §4.3 "Organic divider line". No new string, no new dependency. | `npm run build` clean; no `glow-divider` left in the tree; browser at 1280×800 and 375×812 on `/` (both dividers) and `/impressum` (footer): each line is a soft, wavering thread with varying thickness, different per divider and per reload, drifting slowly and seamlessly (no jump at the loop), gradient + end fades + glow pulse kept, smoke rim still hugs it; no horizontal scroll; no layout shift vs. before (hero/pitch still ≈ 60/40); reduced motion → static; console clean | F20 |
| **F22** | smokemkk-frontend | `frontend/src/components/SmokeIntro.vue`, `frontend/src/views/HomeView.vue` (mount + observer inside `#standorte`, a few lines only), `frontend/src/assets/main.css` only to scope the `html:has(.smoke-intro)` rule — `ui-ux-pro-max` first; §4.3 "Section smoke reveal". No new string, no new dependency. F21 also edits `HomeView.vue`/`main.css`: run F21 and F22 one after the other, never in parallel. | `npm run build` clean; one smoke renderer in the tree; browser at 1280×800 and 375×812 against the running API: load `/` (flag set), scroll to the map → smoke over the section only (header clear) dissolves in ≤ 1.4 s, no rectangular edge visible, then the layer is gone from the DOM and no rAF runs; scroll away and back → no replay; `/impressum` → `/` via SPA → no replay; direct load of `/#standorte` → only the intro, no second smoke; clicks on map/list and Tab work during the dissolve; age-gate backdrop unaffected; reduced motion → nothing; console clean | F11 |
| **F23** | smokemkk-frontend | `frontend/src/components/MachineMap.vue`, `frontend/src/assets/main.css` (group-pin style only) — `ui-ux-pro-max` first; §4.3 "UX audit 2" item 1 (select zoom, grouping); §6 `map.cluster`. No new dependency, no plugin. | `npm run build` clean; against the running API at 1280×800 and 375×812: the overview shows count pins where sites overlap (Bad Orb one `5x`, its tooltip `5 Standorte` then five names on five lines; at the phone zoom also the Wächtersbach/Hesseldorf and Brachttal groups) and every rendered pin is ≥ 28 px from the next (measure the `.machine-marker` centres); click a group → zooms until it splits; select Bad Orb Jahnstraße from the list → zoom ≥ 15 and every other pin ≥ 40 px away; selected colour and nearest ring survive grouping (emulate a position near Bad Orb); groups focusable, Enter zooms, tooltip lists the names; single-pin tooltips show the street with house number on a second line (Fulda → `Niesiger Str. 69`); drop-in still plays once, no replay on zoom; console clean | F22 |
| **F24** | smokemkk-frontend | `frontend/src/components/Reveal.vue`, `frontend/src/views/HomeView.vue`, `frontend/src/components/InventoryPanel.vue`, `frontend/src/views/LegalView.vue` — `ui-ux-pro-max` first; §4.3 "UX audit 2" items 2–7; §6 `map.skip`, `geo.locating`, `geo.unavailable`, visible `search.count` (absorbs the requested F26). No new dependency. | `npm run build` clean; 375×812 against the running API: land on `#standorte` via the caret → map and panel visible at once (block opacity 1 with ≤ 20 % of it on screen); tap a pin and a list row → map card top at 80 px, pin and the detail header on screen; 1280×800 unchanged; keyboard: from the map container the next Tab shows `Karte überspringen`, Enter focuses the search field (list) or `Alle Automaten` (detail); CDP `Emulation.setGeolocationOverride {}` then CTA → `Standort wird ermittelt …` then `Kein Standort freigegeben. Such per PLZ oder Ort.` under the search field and in the status line; a granted position clears it and marks the nearest; query `636` → visible `14 Standorte` under the field (with the geo line below it when both apply), `xyz` → only `search.noResults`, empty query → no count line; h1 word visible ≥ 65 % of a 4 s cycle (sample every 100 ms); `/datenschutz` at 320 and 375: no horizontal scroll; console clean | F23 |
...
- ~~Marker clustering → when > ~50 machines~~ → pulled into F23 (2026-09-26): five sites in one town overlapped at every reachable zoom; density was the trigger, not the count. Own minimal grouping in `MachineMap.vue`, no plugin; Leaflet.markercluster only if spiderfying is ever wanted.
