...
*Status: T0, B1–B3, F1–F13 done and committed (F10 = UX audit + owner additions, aurora-everywhere still a trial; F11–F13 = smoke intro, scroll/divider smoke, bigger hero headline, 2026-09-26; B4 = 24 sites from the Vendon export, 2026-09-26, seed checked on a fresh DB; B5 + F15 = one pin per location with a machine picker, 24 locations / 39 machines, 2026-09-26; F16–F17 = stock list gap to scrollbar and flush sticky headings, F19 = taller map + panel on desktop, 2026-09-26; F22 = section smoke reveal over #standorte, held before the map shows, plus the sprite edge fix, 2026-09-26) · API + site verified end to end without Docker (local Postgres 16 on 5433, see README) · T2 compose run still blocked on WSL · Owner: the PM/orchestrator (`.claude/agents/smokemkk-pm.md`).*
...
- **UX audit changes (F10, 2026-09-25), these override the older bullets below where they differ:**
  ...
  - *Organic divider line (owner request, 2026-09-26: "maybe the line should be more organic instead of straight"):* all three dividers' 2 px straight line becomes a softly wavering thread, like a drawn-out wisp of smoke:
    ...
    - **F21 outcomes accepted by the PM (2026-09-26):** period P 800–1200 px (random), all harmonics whole cycles per P, path sampled every 8 px and 1.5 × the largest screen side + P wide (`// ponytail:` ceiling), drift 12–20 px/s in a random direction; the fixed-in-place gradient is two copies of the thread (violet with the end fades, rose masked 20/50/80 % on top) drifting in sync; the glow sits *under* the thread (on top it washed out the hairline stretches); the rim strips now overlap across the line (`--rim-in: 6px`, fading in over 12 px) because without the old straight line their meeting edge showed as a hard seam. Rainbow bands above the pitch ↔ `#standorte` divider come from the aurora-everywhere TRIAL, not from the dividers.
...
- **Look:** the intro's look scaled to the section box: puffs blown outward from the section's centre, growing and thinning, ≈ 1.2–1.4 s, section legible by ~1 s; fewer puffs than the intro (≈ 20–24) sized relative to the section. **No visible rectangle:** the section sits on the aurora (trial) and the page background, so a flat opaque base would show hard edges — the base (if kept) and the smoke fade softly towards all four section edges (mask), or the base is dropped if the puffs alone read as a cloud. Same randomness rules as the intro.
